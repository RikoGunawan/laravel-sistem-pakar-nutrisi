...
<style>
    ...
    @media (max-width: 768px) {
        .trace-header  { padding: 20px; }
        .trace-step    { padding: 15px; }
        .trace-content { margin-left: 0; }
    }
</style>
@endsection
...
<div class="card">
    ...
    @foreach ($metodeList as $index => $am)
        @php
            ...
            // Rule yang diterapkan (am->rule) + rule lain yang kalah prioritas dari trace
            $tracesMetode = $analisis->tracePenalaran
                ->filter(fn($t) => str_contains($t->proses, $metode->name));

            $rulesEvaluasi = collect();
            foreach ($tracesMetode as $t) {
                $rulesEvaluasi->push([
                    'kode'    => $t->rule_used,
                    'applied' => !str_contains($t->proses, 'tidak diterapkan'),
                    'proses'  => $t->proses,
                ]);
            }

            // Pastikan rule utama selalu masuk jika belum ada
            if ($rule && $rulesEvaluasi->where('kode', $rule->kode_rule)->isEmpty()) {
                $rulesEvaluasi->prepend([
                    'kode'    => $rule->kode_rule,
                    'applied' => true,
                    'proses'  => '',
                ]);
            ...
            $nutrisiTampil = collect($nutrisiHasil)
            ...
            <div class="trace-content">
                ...
                <div>{{ $analisis->makanan->name }}
                    @if($analisis->makanan->kategori)
                        &nbsp;·&nbsp; Kategori: {{ $analisis->makanan->kategori }}
                    @endif
                </div>
        ...
        <div class="trace-step">
            ...
            <div class="trace-content">
                @if ($rule)
                    ...
                    <div><span class="badge badge-active">{{ $rule->kode_rule }}</span></div>
                            ...
                            @foreach ($perubahan as $key => $pct)
                                @if ($pct != 0)
                                    <div class="nutrisi-chip">
                                        ...
                                        <strong style="color: {{ $pct > 0 ? '#dc2626' : '#16a34a' }}">
                                            {{ $pct > 0 ? '+' : '' }}{{ $pct }}%
                                        </strong>
                                    </div>
                                @endif
